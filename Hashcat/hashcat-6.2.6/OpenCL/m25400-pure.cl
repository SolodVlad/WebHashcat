...
KERNEL_FQ void m25400_loop (KERN_ATTR_TMPS_ESALT (pdf14_tmp_t, pdf_t))
{
  ...
  for (u32 i = 0, j = LOOP_POS; i < LOOP_CNT; i++, j++)
  {
    if (j < 50)
    {
      ...
      //  see: "Algorithm 3.3 Computing the encryption dictionary’s O (owner Value) value"
      u32 w0_t[4];
      u32 w1_t[4];
      u32 w2_t[4];
      u32 w3_t[4];
  ...
  // we decrypt the o-value to obtain either the owner-Value (or user-Value if no owner-Value is set)
  //  see: "Algorithm 3.3 Computing the encryption dictionary’s O (owner Value) value": "If there is no owner Value, use the user Value instead".
  u32 tmp[4];
  for (u32 i = 19; i>0; i--)
  {
  ...
  rc4_next_16 (S, 0, 0, out, out, lid); // output of the rc4 decrypt of the o-value should be the padded user-Value
...
KERNEL_FQ void m25400_comp (KERN_ATTR_TMPS_ESALT (pdf14_tmp_t, pdf_t))
{
  ...
  for (int i = 0; i < 16; i++)
  {
    ...
    // we don't use the user-Value in the attack now (as we don't need it),
